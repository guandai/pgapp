(function() {
  var movetime = 5000,
    playtime = 20000,
    fps = 20, // how fast of detect update event
  ...
  HTMLDivElement.prototype.getpos = function(prop) {
    return orgprop = parseFloat(this.style[prop].replace("px", ""))
  }
  ...
  function update() {
    var dropmoveItv = setInterval(function() {
      if (gameEndFlag == 1) {
        ...
        // if (bg1.getpos('left') <= -bgwidth) {
        //   bg1.style.left = bgwidth + "px"
        // } else {
        //   bg1.stepmove("left", -movespeed)
        // }
        // if (bg2.getpos('left') <= -bgwidth) {
        //   bg2.style.left = bgwidth + "px"
        // } else {
        //   bg2.stepmove("left", -movespeed)
        // }


        //tubemove
        $("div[id^=tube]").each(function(index, ele) {
          this.stepmove('left', -movespeed)
          if (this.getpos.left <= -tubewidth) this.parentNode.removeChild(this);
        })
